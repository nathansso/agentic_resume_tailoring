...
def test_dockerfile_cmd_uses_textual_web():
    content = DOCKERFILE.read_text(encoding="utf-8")
    assert "textual-web" in content, "Dockerfile CMD must invoke textual-web"


def test_textual_web_in_requirements_core():
    ...
    assert "textual_web" in pkgs, "textual-web must be listed in requirements-core.txt"
