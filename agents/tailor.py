...
from agents import checks as _checks
from agents.checks import (  # re-exported: tests and callers import these from here
    FAITHFULNESS_MIN, MAX_EXP_BULLETS, MAX_TERM_MENTIONS, MIN_EXP_BULLETS,
    PINNED_SECTIONS, REORDERABLE_SECTIONS, _CONTENT_GATED_SECTIONS,
)
...
class ResumeTailorAgent:
    ...
    _score_and_budget_experiences = staticmethod(_checks.score_and_budget_experiences)

    _enforce_bullet_budgets = staticmethod(_checks.enforce_bullet_budgets)
    ...
    _exp_key = staticmethod(_checks.exp_key)

    _proj_key = staticmethod(_checks.proj_key)

    _label_for_key = staticmethod(_checks.label_for_key)
    ...
    _rendered_by_key = staticmethod(_checks.rendered_by_key)

    _faithfulness_drift = staticmethod(_checks.faithfulness_drift)

    _over_repeated_terms = staticmethod(_checks.over_repeated_terms)
    ...
    _expected_sections = staticmethod(_checks.expected_sections)
    ...
    _annotate_with_action = staticmethod(_checks.annotate_with_action)

    _apply_plan_to_inputs = staticmethod(_checks.apply_plan_to_inputs)

    _enforce_plan = staticmethod(_checks.enforce_plan)
