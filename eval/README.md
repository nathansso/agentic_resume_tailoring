...
| `cassettes/` | 0 | replay mode | **None committed.** #182 deleted the #158 recording rather than refresh it: its three task ids came from the corpus #177 replaced and its profile was retired by #172. Produce one with `python eval/tailoring_benchmark.py --mode product --record --tasks <id> <id> …` — pass `--tasks` explicitly, since `--limit` takes an alphabetical prefix of the corpus. Both replay tests skip while the directory is empty, and say so |
